...
public class GlideEngine implements ImageEngine {
    ...
    public void loadImg(Context context, Object path, ImageView imageView, boolean isNeedPlaceHolder, final AlbumEngineLoadListener... listeners) {
        BitmapRequestBuilder builder = Glide.with(context).load(path).asBitmap().error(com.yu.bundles.album.R.mipmap.mae_album_img_default);
        if(isNeedPlaceHolder){
            builder = builder.placeholder(com.yu.bundles.album.R.mipmap.mae_album_img_default);
        }
    ...
    public void loadGifImg(Context context, Object path, ImageView imageView, boolean isNeedPlaceHolder, final AlbumEngineLoadListener... listeners) {
        GifRequestBuilder builder = Glide.with(context).load(path).asGif().priority(Priority.HIGH).diskCacheStrategy(DiskCacheStrategy.NONE).error(com.yu.bundles.album.R.mipmap.mae_album_img_default);
        if(isNeedPlaceHolder){
            builder = builder.placeholder(com.yu.bundles.album.R.mipmap.mae_album_img_default);
        }
        if(MethodUtils.isNull(listeners)){
