...
public final class CameraManager {
	...
	private static int findDesiredDimensionInRange(int resolution, int hardMin,
												   int hardMax) {
		int dim = (int) (resolution * MAEScannerParams.getInstance().rectEdgeWidthRate); // Target 5/8 of each dimension
		if (dim < hardMin) {
			return hardMin;
		}
		if (dim > hardMax) {
	...
	public synchronized Rect getFramingRect() {
		if (framingRect == null) {
			...
			// 将扫描框设置成一个正方形
			int height = (int) (width * MAEScannerParams.getInstance().rectEdgeHeightRateToWidth);
	...
	private synchronized Rect getFramingRectInPreview() {
		if (framingRectInPreview == null) {
			...
			/*Logger.d(TAG, "Calculated framingRectInPreview rect: "
					+ framingRectInPreview);
			Logger.d(TAG, "cameraResolution: " + cameraResolution);
			Logger.d(TAG, "screenResolution: " + screenResolution);*/
		}
