...
final class DecodeHandler extends Handler {
	...
	public void handleMessage(Message message) {
		...
		if (message.what == R.id.mae_bundles_scanner_decode) {
			decode((byte[]) message.obj, message.arg1, message.arg2);
		} else if (message.what == R.id.mae_bundles_scanner_quit) {
			running = false;
		...
		if(handler != null){
			if(rawResult != null){
				Message message = Message.obtain(handler,
						R.id.mae_bundles_scanner_decode_succeeded, rawResult);
				Bundle bundle = new Bundle();
				bundleThumbnail(source, bundle);
				message.setData(bundle);
				message.sendToTarget();
			} else {
				Message message = Message.obtain(handler, R.id.mae_bundles_scanner_decode_failed);
				message.sendToTarget();
			}
		}
	}
